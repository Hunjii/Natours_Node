...
exports.signup = catchAsync(async (req, res, next) => {
  if (req.body.role === 'admin')
    return next(new AppError('Please change role!', 400));
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // 4) Check user changed password or token has expired
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError('There are an error sending email. Try again later!')
    );
  }
});
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) Check expired token, and user
  if (!user) {
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(
      new AppError('Your current password not correct. Please try again!', 401)
    );
  }
